...
public class BusinessSell extends AppCompatActivity{
    private static final String TAG = "BusinessSell";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = FirebaseDatabase.getInstance().getReference().child("ToSell");
        ...
        final ArrayList<String> matval = new ArrayList<>();
        ref = FirebaseDatabase.getInstance().getReference().child("Materials");
        FirebaseRecyclerAdapter<Material, MaterialHolder> recyclerAdapter1 = new FirebaseRecyclerAdapter<Material, MaterialHolder>
                (Material.class, R.layout.business_material_info2, MaterialHolder.class, ref) {
            @Override
                    ...
                    public void onCheckedChanged(final CompoundButton compoundButton, final boolean b) {
                        if(compoundButton.isChecked()) {
                            compoundButton.setEnabled(false);
                                ...
                                FirebaseDatabase.getInstance().getReference().child("ToSell").child(user.getUid())
                                            ...
                                            public void onDataChange(DataSnapshot dataSnapshot) {
                                                for (DataSnapshot sp : dataSnapshot.getChildren()) {
                                                    ...
                                                    if (materialName.equals(sp.child("matname").getValue())) {
                                                        ...
                                                        Toast.makeText(BusinessSell.this, materialName + " already displayed", Toast.LENGTH_SHORT).show();
                                                        flag = false;
                                                        fake = true;
...
//                }
                                for (int x = 0; x < materials.size(); x++) {
                                    ...
                                    Map map = new HashMap();
                                    map.put("matname", materials.get(x));
                                    key = db.child("ToSell").child(user.getUid()).push().getKey();
                                    db.child(user.getUid()).child(key).setValue(map);
                                }
                                startActivity(new Intent(BusinessSell.this, BusinessProfile.class));
                            }
